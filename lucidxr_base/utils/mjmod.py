...
import lucidxr_base.utils.transform_utils as trans
...
class LightingModder(BaseModder):
    ...
    def __init__(
        ...
        super().__init__(physics, random_state=random_state)

        if light_idxs is None:
            light_idxs = [*range(self.model.nlight)]
    ...
    def save_defaults(self):
        ...
        """
        self._defaults = {k: {} for k in self.light_idxs}
        for name in self.light_idxs:
...
class TextureModder(BaseModder):
    ...
    def __init__(
        ...
        # gradient is weird because there are clear bounds to the box
        self.skybox_texture_variations = ["rgb", "checker", "noise"]
        ...
        self._texture_variation_callbacks = {k: self._all_texture_variation_callbacks[k] for k in self.texture_variations}
    ...
    def restore_defaults(self, geom_names=None, skybox=False):
        """
        Reloads the saved parameter values.
        """
        if geom_names is None:
        ...
        if skybox and self.randomize_skybox:
    ...
    def _randomize_geom_color(self, name):
        ...
        if self.randomize_local:
            ...
            rgb = (1.0 - self.local_rgb_interpolation) * self._defaults[name]["rgb"] + self.local_rgb_interpolation * random_color
        else:
    ...
    def _randomize_texture(self, name):
        ...
        if name == "skybox":
            keys = self.skybox_texture_variations
    ...
    def whiten_materials(self):
        ...
        for name in self.geom_names:
            # whiten geom
            geom_id = self.geom_name2id[name]
    ...
    def set_texture(self, name, bitmap, perturb=False):
        ...
        if perturb:
            bitmap = (1.0 - self.local_rgb_interpolation) * self._defaults[name]["texture"] + self.local_rgb_interpolation * bitmap
    ...
    def get_material(self, name):
        ...
        material = np.array((self.model.mat_reflectance[mat_id], self.model.mat_shininess[mat_id], self.model.mat_specular[mat_id]))
        return material
    ...
    def upload_texture(self, name):
        ...
        Args:
            name (str): name of geom
        """
        if self.physics._contexts is None:
            return
        # Note: no idea what this does, but it works
        texture = self.get_texture(name)
        mujoco.mjr_uploadTexture(self.physics.model.ptr, self.physics._contexts[-1].ptr, texture.id)
...
class DynamicsModder(BaseModder):
    ...
    def mod(self, name, attr, val):
        ...
        assert attr in self.dynamics_parameters, (
            "Invalid dynamics parameter specified! Supported parameters are: {};" " requested: {}".format(self.dynamics_parameters, attr)
        )
