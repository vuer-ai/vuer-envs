...
from lucidxr_base.utils.mjmod import CameraModder, DynamicsModder, LightingModder, TextureModder
...
class DomainRandomizationWrapper(MovableCameraWrapper):
    """
    ...
    def __init__(
        ...
        **rest,
        ...
        self.physics = self.unwrapped.lucidenv.physics
        if seed is not None:
    ...
    def reset(self):
        ...
        for modder in self.modders:
            ...
            modder.update_sim(self.physics)

        if self.randomize_on_reset:
            # domain randomize + regenerate observation
            self.randomize_domain()
            ret = self.unwrapped._get_obs()
    ...
    def randomize_domain(self):
        ...
        for modder in self.modders:
            modder.randomize()
            # modder.restore_defaults(self.ignored_geom_names, skybox=False)
