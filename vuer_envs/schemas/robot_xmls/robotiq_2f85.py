...
class Robotiq2f85(Body):
    ...
    <asset>
      ...
      <mesh class="{childclass}" file="base_mount.stl"/>
      <mesh class="{childclass}" file="base.stl"/>
      <mesh class="{childclass}" file="driver.stl"/>
      <mesh class="{childclass}" file="coupler.stl"/>
      <mesh class="{childclass}" file="follower.stl"/>
      <mesh class="{childclass}" file="pad.stl"/>
      <mesh class="{childclass}" file="silicone_pad.stl"/>
      <mesh class="{childclass}" file="spring_link.stl"/>
    </asset>
    ...
    <body name="{name}-base" pos="0 0 0.0038" quat="1 0 0 -1">
      ...
      <body name="{name}-right_driver" pos="0 0.0306011 0.054904">
        ...
        <joint name="{name}-right_driver_joint" class="driver"/>
        ...
        <body name="{name}-right_coupler" pos="0 0.0315 -0.0041">
          ...
          <joint name="{name}-right_coupler_joint" class="coupler"/>
      ...
      <body name="{name}-right_spring_link" pos="0 0.0132 0.0609">
        ...
        <joint name="{name}-right_spring_link_joint" class="spring_link"/>
        ...
        <body name="{name}-right_follower" pos="0 0.055 0.0375">
          ...
          <joint name="{name}-right_follower_joint" class="follower"/>
          ...
          <body name="{name}-right_pad" pos="0 -0.0189 0.01352">
            <geom class="pad_box1" name="{name}-right_pad1"/>
            <geom class="pad_box2" name="{name}-right_pad2"/>
      ...
      <body name="{name}-left_driver" pos="0 -0.0306011 0.054904" quat="0 0 0 1">
        ...
        <body name="{name}-left_coupler" pos="0 0.0315 -0.0041">
          <inertial mass="0.0140974" pos="0 0.00301209 0.0232175" quat="0.705636 -0.0455904 0.0455904 0.705636"
            diaginertia="4.16206e-06 3.52216e-06 8.88131e-07"/>
          <joint name="{name}-left_coupler_joint" class="coupler"/>
          <geom class="visual" mesh="coupler" material="black"/>
          <geom class="collision" mesh="coupler"/>
        </body>
      </body>
      <body name="{name}-left_spring_link" pos="0 -0.0132 0.0609" quat="0 0 0 1">
        <inertial mass="0.0221642" pos="-8.65005e-09 0.0181624 0.0212658" quat="0.663403 -0.244737 0.244737 0.663403"
          diaginertia="8.96853e-06 6.71733e-06 2.63931e-06"/>
        <joint name="{name}-left_spring_link_joint" class="spring_link"/>
        <geom class="visual" mesh="spring_link" material="black"/>
        <geom class="collision" mesh="spring_link"/>
        <body name="{name}-left_follower" pos="0 0.055 0.0375">
          <inertial mass="0.0125222" pos="0 -0.011046 0.0124786" quat="1 0.1664 0 0"
            diaginertia="2.67415e-06 2.4559e-06 6.02031e-07"/>
          <joint name="{name}-left_follower_joint" class="follower"/>
          <geom class="visual" mesh="follower" material="black"/>
          <geom class="collision" mesh="follower"/>
          <body name="{name}-left_pad" pos="0 -0.0189 0.01352">
            <geom class="pad_box1" name="{name}-left_pad1"/>
            <geom class="pad_box2" name="{name}-left_pad2"/>
    ...
    -->
    <actuator>
      <general class="{childclass}" name="{name}-fingers_actuator" tendon="split" forcerange="-5 5" ctrlrange="0 255"
